...
def ode_e(y, e, Omega, sigma, m, A):
  '''
    an ode function, finds the change in the state vector, y, for each incremental change in e. 
    Used to find the path that the vessel will take the state vector is broken up into its components, 
    V, D, and L are the velocity, Drag acceleration, and Lift acceleration magnitudes
    Parameters:
...
def ode_t(y, Omega, sigma, m, A):
  ...
  ''' y = [r-dot, theta-dot, phi-dot, gamma-dot, psi-dot, s-dot] '''
  y = [r     + dt*(V * sin(gamma)),
...
def find_minimum_sigma(greatCircle, y_0):
  '''
...
def print_path(sol, rList, sList):
  '''  
      plots the vessel's path, along with the Earth's surface
      xList0 is the path integrated with respect to e, xList 1 is
      the path integrated with respect to time
  '''
  xList0, yList0 = [], []
  for s in sol:
    r, s = s[0], s[5]
...
sigma           = find_minimum_sigma(greatCircle, y_0)
sol, _          = solve_for_sigma(sigma, y_0, greatCircle) 
t, rList, sList = find_time_descent(y_0, sol[-1])
t=t*dt

print("Time of descent:", t)

print_path(sol, rList, sList)
